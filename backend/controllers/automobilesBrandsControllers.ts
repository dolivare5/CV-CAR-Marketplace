/* Importing the Request and Response objects from the express module. */
/* Importación de los objetos Solicitud y Respuesta desde el módulo express. */
 ...
 */
const getAutomobilesBrands = async (_req: Request, res: Response) => {
    ...
    const automobilesBrands: AutomobilesBrands[ ]= await AutomobilesBrands.findAll();
 ...
 */
const getAutomobilesBrandsById = async (req: Request, res: Response) => {
    const { AutBrand_Id } = req.params;
    const automobilesBrands: AutomobilesBrands | null = await AutomobilesBrands.findByPk(AutBrand_Id);
    return res.json({
 ...
 */
const postAutomobilesBrands = async (req: Request, res: Response) => {
    ...
    try{
        ...
        const resultsValidations : Object[] = await validationsAutomobilesBrands(req, res);
        ...
        const existAutomobilesBrands: AutomobilesBrands | null = await AutomobilesBrands.findOne({
            where: {
        ...
        /* It's creating a new record in the `AutomobilesBrands` table. */
        /* Está creando un nuevo registro en la tabla `AutomobilesBrands`. */
        const automobilesBrands: AutomobilesBrands = await AutomobilesBrands.create(body);
...
const putAutomobilesBrands = async (req: Request, res: Response) => {
    /* The data sent by the client is obtained. */
    /* Se extraen los datos del cuerpo de la solicitud. */
    const { body } = req;
    const { AutBrand_Id } = req.params;
    try{
        ...
        const existAutomobilesBrands : Object[] = await db.query(
            `SELECT * FROM automobiles_brands WHERE AutBrand_Name = '${AutBrand_Name}' AND AutBrand_Id != ${AutBrand_Id}`,
            {
 ...
 */
const validationsAutomobilesBrands = async (req: Request, _res: Response) => {
    /* It's a validation that checks if the `AutBrand_Name` field is empty, and if it is, it returns a message. */
    /* Es una validación que comprueba si el campo `AutBrand_Name` está vacío y, si lo está, devuelve un mensaje. */
...
export {
    getAutomobilesBrands,
    getAutomobilesBrandsById,
    postAutomobilesBrands,
    putAutomobilesBrands
}